...
df = pd.read_csv("5.hafta Recommender_system/ödevler/armut_data.csv")
df.head()
df.describe().T
df.isnull().sum()
df.shape
...
def create_pivot_df(dataframe):
    return dataframe.groupby(['SepetID', "service"])['service'].count().unstack().fillna(0). \
            applymap(lambda x: 1 if x > 0 else 0)

df_pivot = create_pivot_df(df)
df_pivot.head()
# Adım 2: Birliktelik kurallarını oluşturunuz.
frequent_itemsets = apriori(df_pivot, min_support=0.01, use_colnames=True) #olası ürün birliklerinin olasılıklarını verir.
#analiz etmek için supporta göre sıralayalım
frequent_itemsets.sort_values("support", ascending=False) #supprta göre azalan bir şekilde sırala
#bu çıktılar herbir ürünün olasılığıdır. bize lazım olan ise birliktelik kuralı
#birliktelik kurallarını çıkaralım
